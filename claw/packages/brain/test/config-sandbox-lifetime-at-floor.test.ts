 ...
 * configured.
 *
 * The refusing side is config-sandbox-lifetime-too-short.test.ts, one second
 * below. Neither file means much alone: a check that refuses everything passes
 * that one, a check that refuses nothing passes this one, and only the pair
 * pins the boundary where the doc comment's arithmetic puts it.
 */
...
// derivation adds for the 2s write timeout and agentd's 1s requeue grace.
