...
describe("a batch commits its admitted prefix, or nothing", { skip }, () => {
  ...
  before(async () => {
    ...
    app = Fastify();
    app.addHook("preHandler", async (req) => { (req as unknown as { user: unknown }).user = SUBMITTER; });
    await tasks.registerBatchRoute(app);
    await app.ready();
  });
  after(async () => {
    await app?.close();
  ...
  const submit = (inputs: Array<Record<string, unknown>>) => app.inject({
    method: "POST",
    url: "/v1/batches",
  ...
  const scalar = async (sql: string, params: unknown[] = []) => {
    const r = await harness.app.db.db.query(sql, params);
    return r.rows[0] as Record<string, unknown> | undefined;
  };

  const count = async (sql: string, params: unknown[] = []) =>
    Number((await scalar(sql, params))!.n);

  test("a three-input batch refused on its third input commits the first two", async () => {
    await reset();

    const res = await submit([{ i: 1 }, { i: 2 }, { i: 3 }]);
    ...
    assert.equal(await count("SELECT COUNT(*)::int AS n FROM claw_batches"), 1);

    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_tasks WHERE batch_id = $1", [body.batch_id]),
      2,
      "one brain node per admitted input, and none for the third",
    );
    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_tasks"),
      4,
      "two virtual DAG roots plus their two nodes",
    );

    const roots = await harness.app.db.db.query(
  ...
  test("a batch refused on its first input writes no batch row and no credential", async () => {
    ...
    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_batches"),
      0,
      "the provisional INSERT was rolled back with the refusal",
    );
    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_batches WHERE session_id = $1", ["s-batch"]),
      0,
    );
    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_batches WHERE size = 0"),
      0,
    ...
    assert.equal(
      await count("SELECT COUNT(*)::int AS n FROM claw_tasks WHERE dag_id = 'one-node'"),
      0,
    );
  });
  ...
  test("a batch every input of which is admitted keeps size at the requested count", async () => {
    ...
    const res = await submit([{ i: 1 }, { i: 2 }]);
    ...
    const batch = await scalar("SELECT size FROM claw_batches WHERE batch_id = $1", [body.batch_id]);
