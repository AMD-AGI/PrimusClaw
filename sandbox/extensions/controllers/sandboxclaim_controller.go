...
// TODO: These constants should be imported from the main controller package.
const (
...
func (r *SandboxClaimReconciler) updateStatus(ctx context.Context, oldStatus *extensionsv1alpha1.SandboxClaimStatus, claim *extensionsv1alpha1.SandboxClaim) error {
	...
	if err := r.Status().Update(ctx, claim); err != nil {
		if k8errors.IsConflict(err) {
			log.V(1).Info("sandboxclaim status update conflict; will retry on next reconcile")
			return nil
		}
...
func (r *SandboxClaimReconciler) tryAdoptPodFromPool(ctx context.Context, claim *extensionsv1alpha1.SandboxClaim, sandbox *v1alpha1.Sandbox) (*corev1.Pod, error) {
	...
	for i := range podList.Items {
		...
		// Skip pods in terminal failure state (CrashLoopBackOff, ErrImagePull, etc.)
		// — don't adopt broken pods for sandbox creation.
		if isPodTerminallyFailed(pod) {
			log.Info("Skipping failed warm pool pod", "pod", pod.Name)
			continue
		}
...
func (r *SandboxClaimReconciler) createSandbox(ctx context.Context, claim *extensionsv1alpha1.SandboxClaim, template *extensionsv1alpha1.SandboxTemplate) (*v1alpha1.Sandbox, error) {
	...
	// TODO: this is a workaround, remove replica assignment when upstream supports it.
	replicas := int32(1)
...
func (r *SandboxClaimReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Allow parallel reconciliation of multiple SandboxClaims.
		// Default is 1 (serial), which causes queuing under concurrent sandbox creation.
		// 10 concurrent workers allows up to 10 sandboxes to be provisioned simultaneously.
