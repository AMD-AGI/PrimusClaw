...
// These tests require a live Kubernetes cluster, reachable via a kubeconfig at
// `bin/KUBECONFIG` (see test/e2e/framework/context.go). They are excluded from
// `go test ./...` so a clean clone passes without cluster credentials.
// Run them with: make test-e2e   (or: go test -tags e2e ./test/e2e/...)
