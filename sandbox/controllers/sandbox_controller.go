...
const (
	...
	annSessionID = "runtime.agent-sandbox.io/session-id"
	labelUserID  = "runtime.agent-sandbox.io/user.id"
	annUserName  = "runtime.agent-sandbox.io/user.name"
	// Set after a successful shutdown-expiry audit so retries do not duplicate events.
	annShutdownExpiryAuditIssued = "runtime.agent-sandbox.io/shutdown-expiry-audit-issued"
)
...
type SandboxReconciler struct {
	...
	// Audit records sandbox.deleted when ShutdownTime expires (nil skips, e.g. memory store).
...
func (r *SandboxReconciler) updateStatus(ctx context.Context, oldStatus *sandboxv1alpha1.SandboxStatus, sandbox *sandboxv1alpha1.Sandbox) error {
	...
	if err := r.Status().Update(ctx, sandbox); err != nil {
		if k8serrors.IsConflict(err) {
			log.V(1).Info("status update conflict; will retry on next reconcile")
			return nil
		}
...
// emitShutdownExpiryAudit records sandbox.deleted when the runtime controller tears
// down resources because spec.lifecycle.shutdownTime has passed. Fire-and-forget:
// failures are logged and never block expiry cleanup.
func (r *SandboxReconciler) emitShutdownExpiryAudit(ctx context.Context, sandbox *sandboxv1alpha1.Sandbox) {
	if r.Audit == nil {
		return
	}
	if sandbox.Annotations != nil && sandbox.Annotations[annShutdownExpiryAuditIssued] != "" {
		return
	}
	log := log.FromContext(ctx)
	sessionID := ""
	...
	event := &audit.AuditEvent{
		...
		UserName:     userName,
		Timestamp:    time.Now(),
		DeleteReason: audit.ReasonShutdownExpired,
	}
	if !created.IsZero() {
	...
	if err := r.Audit.Store(ctx, event); err != nil {
		log.Error(err, "failed to emit shutdown expiry audit", "sandbox", sandbox.Name, "namespace", sandbox.Namespace)
		return
	}
...
func (r *SandboxReconciler) handleSandboxExpiry(ctx context.Context, sandbox *sandboxv1alpha1.Sandbox) (bool, error) {
	r.emitShutdownExpiryAudit(ctx, sandbox)
...
func (r *SandboxReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		Named("sandbox-runtime-controller").
